...
l_sym2num = {"S": 0, "P": 1, "D": 2, "F": 3, "G": 4, "H": 5, "SP": [0, 1]}
    ...
    if "gbasis" in inputattr:
        ...
        for idx, i in enumerate(uatoms_idx):
            ...
            for jdx, j in enumerate(curr_atom_basis):
                ...
                for k in curr_e_prim:
                    new_list.append(k)
                if not "{}".format(qe.periodictable.to_E(uatoms[idx])) in basis:
                    basis["{}".format(qe.periodictable.to_E(uatoms[idx]))] = [new_list]
                else:
                    basis["{}".format(qe.periodictable.to_E(uatoms[idx]))].append(
                        new_list
                    )
        mol.basis = basis
    return mol
